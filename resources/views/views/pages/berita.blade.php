...
<div class="flex flex-col md:flex-row gap-gutter mt-4">
<!-- Search -->
<div class="relative flex-grow">
<span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline">search</span>
<input class="w-full bg-surface pl-12 pr-4 py-4 rounded-xl border border-outline-variant focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-body-md text-body-md text-on-surface placeholder:text-outline-variant shadow-level-1" placeholder="Cari artikel, topik, atau kegiatan..." type="text"/>
</div>
<!-- Categories -->
...
<button class="whitespace-nowrap px-6 py-3 {{ $index === 0 ? 'bg-primary text-on-primary shadow-level-1' : 'bg-surface text-primary border border-primary/20 hover:bg-primary-container/10' }} rounded-full font-label-md text-label-md transition-colors flex-shrink-0">{{ $label }}</button>
@endforeach
</div>
</div>
</section>
...
@if($featuredPost)
<article class="lg:col-span-8 bg-surface rounded-2xl shadow-level-1 overflow-hidden group cursor-pointer hover:shadow-level-2 transition-shadow duration-300">
...
@if($secondaryPost)
<article class="flex-1 bg-surface rounded-2xl shadow-level-1 overflow-hidden group cursor-pointer hover:shadow-level-2 transition-shadow duration-300 flex flex-col">
...
<article class="bg-surface rounded-2xl shadow-level-1 overflow-hidden group cursor-pointer hover:shadow-level-2 transition-shadow duration-300 flex flex-col">
<div class="relative h-48 w-full overflow-hidden">
...
@empty
<article class="md:col-span-3 bg-surface rounded-2xl border border-outline-variant p-6 text-on-surface-variant">Belum ada artikel berita.</article>
@endforelse
</div>
</section>
</main>
